...
from oiqa_bpr_vmamba.cli.common import run_logged_subprocess, write_grouped_metrics_table
...
def main() -> None:
    ...
    if not args.skip_main:
        ...
        result = run_logged_subprocess(cmd, main_dir / 'train.log')
    ...
    if not args.skip_ablation:
        ...
        cmd = [sys.executable, '-m', 'oiqa_bpr_vmamba.cli.run_ablation', '--config', args.config, '--work-dir', str(ablation_dir), '--include-baseline']
        cmd = _append_common_args(cmd, args)
        print('Running:', ' '.join(cmd))
        result = run_logged_subprocess(cmd, ablation_dir / 'run_ablation.log')
    ...
    if not args.skip_split_protocols:
        ...
        cmd = _append_common_args(cmd, args)
        print('Running:', ' '.join(cmd))
        result = run_logged_subprocess(cmd, splits_dir / 'run_split_protocols.log')

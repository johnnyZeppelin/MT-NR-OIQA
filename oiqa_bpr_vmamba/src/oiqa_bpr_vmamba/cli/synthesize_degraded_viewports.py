    ...
    for _, row in tqdm(manifest.iterrows(), total=len(manifest), desc='synthesize degraded viewports'):
        for vp_col, deg_col in zip(vp_cols, deg_cols):
            ...
            try:
                ...
                warnings.warn(
                    f'Manifest destination {dst} is not under --output-root {out_root}. '                     'The file will still be written to the manifest path so training can find it.'
                )
            dst.parent.mkdir(parents=True, exist_ok=True)
            if dst.exists():
                continue
            img = Image.open(src).convert('RGB')
            seed = stable_int_hash(row['image_id'], vp_col, args.seed)

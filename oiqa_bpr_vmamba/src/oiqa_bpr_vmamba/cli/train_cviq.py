...
def main() -> None:
    ...
    if resume_path is not None:
        ...
        ckpt_best_metric_name = str(ckpt.get('best_metric_name', best_metric_name)).upper()
        if ckpt_best_metric_name != best_metric_name:
            warnings.warn(
                f'Resuming checkpoint monitored with {ckpt_best_metric_name}, but current run expects {best_metric_name}. '
                'Best-metric tracking will continue using the current setting.'
            )
